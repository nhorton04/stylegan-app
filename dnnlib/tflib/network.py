...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with tf.name_scope(None):
            self.scope = tf.compat.v1.get_default_graph(
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE), tfutil.absolute_name_scope(self.scope):
            ...
            with tf.control_dependencies(None):
                self.input_templates = [tf.compat.v1.placeholder(
        ...
        self.input_shapes = [tfutil.shape_to_list(
            t.shape) for t in self.input_templates]
        self.output_shapes = [tfutil.shape_to_list(
            t.shape) for t in self.output_templates]
        ...
        self.own_vars = OrderedDict((var.name[len(
            self.scope) + 1:].split(":")[0], var) for var in tf.compat.v1.global_variables(self.scope + "/"))
    ...
    def apply_swa(self, src_net, epoch):
        ...
        tfutil.set_vars(tfutil.run({self.vars[name]: (
            src_net.vars[name] * scale_new_data + self.vars[name] * scale_moving_average) for name in names}))
    ...
    def run(self,
            ...
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.control_dependencies(None):
                if custom_inputs is not None:
                    with tf.device("/gpu:0"):
                        in_expr = [input_builder(name) for input_builder, name in zip(
                            custom_inputs, self.input_names)]
                        in_split = list(
                            zip(*[tf.split(x, num_gpus) for x in in_expr]))
                else:
                    with tf.device("/cpu:0"):
                        in_expr = [tf.placeholder(
                            tf.float32, name=name) for name in self.input_names]
                        in_split = list(
                            zip(*[tf.split(x, num_gpus) for x in in_expr]))
        ...
        out_arrays = [np.empty([num_items] + tfutil.shape_to_list(expr.shape)
                               [1:], expr.dtype.name) for expr in out_expr]
        ...
        for mb_begin in range(0, num_items, minibatch_size):
            ...
            mb_out = tf.compat.v1.get_default_session().run(
                out_expr, dict(zip(in_expr, mb_in)))
    ...
    def print_layers(self, title: str = None, hide_layers_with_no_params: bool = False) -> None:
        ...
        for layer_name, layer_output, layer_trainables in self.list_layers():
            num_params = sum(np.prod(tfutil.shape_to_list(var.shape))
                             for var in layer_trainables)
            weights = [var for var in layer_trainables if var.name.endswith(
                "/weight:0") or var.name.endswith("/weight_1:0")]
