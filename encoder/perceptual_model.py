...
class PerceptualModel:
    ...
    def add_placeholder(self, var_name):
        ...
        setattr(self, var_name + "_placeholder",
                tf.placeholder(var_val.dtype, shape=var_val.get_shape()))
        setattr(self, var_name + "_op",
    ...
    def build_perceptual_model(self, generator, discriminator=None):
        ...
        if (self.vgg_loss is not None):
            vgg16 = VGG16(include_top=False, input_shape=(
                self.img_size, self.img_size, 3))
            self.perceptual_model = Model(
                vgg16.input, vgg16.layers[self.layer].output)
        ...
        if self.perc_model is not None and self.lpips_loss is not None:
            img1 = tflib.convert_images_from_uint8(
                self.ref_weight * self.ref_img, nhwc_to_nchw=True)
            img2 = tflib.convert_images_from_uint8(
                self.ref_weight * generated_image, nhwc_to_nchw=True)
        ...
        if (self.vgg_loss is not None):
            if self.adaptive_loss:
                self.loss += self.vgg_loss * tf_custom_adaptive_loss(
                    self.features_weight * self.ref_img_features, self.features_weight * generated_img_features)
            else:
        ...
        if (self.mssim_loss is not None):
            self.loss += self.mssim_loss * tf.math.reduce_mean(1-tf.image.ssim_multiscale(
                self.ref_weight * self.ref_img, self.ref_weight * generated_image, 1))
        # + extra perceptual loss on image pixels
        if self.perc_model is not None and self.lpips_loss is not None:
            self.loss += self.lpips_loss * \
                tf.math.reduce_mean(self.perc_model.get(img1, img2))
        # + L1 penalty on dlatent weights
        if self.l1_penalty is not None:
            self.loss += self.l1_penalty * 512 * \
                tf.math.reduce_mean(tf.math.abs(
    ...
    def generate_face_mask(self, im):
        ...
        for (j, rect) in enumerate(rects):
            ...
            if self.use_grabcut:
                ...
                cv2.grabCut(im, mask, rect, bgdModel,
    ...
    def set_reference_images(self, images_list):
        ...
        loaded_image = load_images(
            images_list, self.img_size, sharpen=self.sharpen_input)
        image_features = None
        ...
        if self.face_mask:
            ...
            for (i, im) in enumerate(loaded_image):
                try:
                    ...
                    mask = np.ones(im.shape, np.float32) * \
        ...
        if len(images_list) != self.batch_size:
            if image_features is not None:
                ...
                empty_features_shape = [
                    self.batch_size - len(images_list)] + features_space
    ...
    def optimize(self, vars_to_optimize, iterations=200, use_optimizer='adam'):
        vars_to_optimize = vars_to_optimize if isinstance(
            vars_to_optimize, list) else [vars_to_optimize]
        if use_optimizer == 'lbfgs':
            optimizer = tf.contrib.opt.ScipyOptimizerInterface(
                self.loss, var_list=vars_to_optimize, method='L-BFGS-B', options={'maxiter': iterations})
        else:
        ...
        for _ in range(iterations):
            if use_optimizer == 'lbfgs':
                optimizer.minimize(self.sess, fetches=[
                                   vars_to_optimize, self.loss])
